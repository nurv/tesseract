...
(function() {
	...
	from.fn = from.prototype = {
		...
		toArray : function() {
			return map(this.items, function(o) {
				return o
			});
		},
		...
		intersect : function(secondArray, clauseMethod) {
			...
			clauseMethod = clauseMethod
					|| function(item, item2, index, index2) {
						return (item["equals"] && item.equals(item2))
								|| (item2["equals"] && item2.equals(item1))
								|| item == item2;
					};
